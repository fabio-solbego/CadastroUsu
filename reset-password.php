...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["new_senha"]))){
        ...
        $new_password = trim($_POST["new_senha"]);
    }
    ...
    if(empty($new_password_err) && empty($confirm_password_err)){
        ...
        $sql = "UPDATE tbusuario SET senha = :senha WHERE idUsu = :idUsu";
        
        if($stmt = $pdo->prepare($sql)){
            // Vincule as variáveis à instrução preparada como parâmetros
            $stmt->bindParam(":senha", $param_password, PDO::PARAM_STR);
            $stmt->bindParam(":idUsu", $param_id, PDO::PARAM_INT);
            
            // Definir parâmetros
            $param_password = password_hash($new_password, PASSWORD_DEFAULT);
            $param_id = $_SESSION["idUsu"];
...
<body>
    <div class="wrapper">
        <h2>Redefinina sua senha</h2>
        <p>Por favor, preencha aqui para redefinir sua senha</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> 
            <div class="form-group">
                <label>Nova senha</label>
                <input type="password" name="new_senha" class="form-control <?php echo (!empty($new_password_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $new_password; ?>">
                <span class="invalid-feedback"><?php echo $new_password_err; ?></span>
            </div>
            <div class="form-group">
                <label>Confirme a senha</label>
                <input type="password" name="confirm_senha" class="form-control <?php echo (!empty($confirm_password_err)) ? 'is-invalid' : ''; ?>">
